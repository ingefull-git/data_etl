...
def test_request_generic_200(request_args_200, global_config):
    ...
    headers, params, payload = request_args_200
    args = {'entity_name': u'section', 'stream': False, 'url': '/ws/schema/query/com.blackboard.datalink.section', 'params': {'page': 1, 'pagesize': 5}, 'file_headers': [u'organizationid', u'organizationinternalID', u'sectionid', u'sectioninternalID', u'courseid', u'period', u'number', u'teacherid', u'teacherinternalID', u'room', u'url', u'termid'], 'payload': {'yearid': 32}}
...
def test_request_generic_404(request_args_404, global_config):
    ...
    requestretry = RequestRetry(session=session, method='POST', hostname=client.hostname+url, headers=headers, payload=payload, logger=logger)
...
def test_request_generic_timeout_exception(mock_session, request_args_404, response_read_timeout, global_config):
    logger, config_file, session, client, url = global_config
    headers, params, payload = request_args_404
    ...
    requestretry = RequestRetry(logger=logger)
    response = requestretry.make_request('POST', url=url, headers=headers, params=params, payload=payload, stream=False, timeout=600.0)
