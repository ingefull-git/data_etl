...
def test_stream_queries_404(mock_json, mock_json_txt, response_404, global_config):
    ...
    mock_json_txt.return_value = None
    req = stream_queries_request(session, client, client.stream_list, yearid=99)
    assert req.values()[0][0] == u'HTTP error: 404 Client Error: None for url: None'


#@pytest.mark.skip
@mock.patch.object(powerQueriesPull_generic, "json_to_txt")
@mock.patch.object(powerQueriesPull_generic, "stream_to_json")
def test_stream_queries_404_200(mock_json, mock_json_txt, response_404, response_200, global_config):
    logger, config_file, session, client = global_config
...
@mock.patch.object(powerQueriesPull_generic, "json_to_txt")
@mock.patch.object(powerQueriesPull_generic, "stream_to_json")
def test_stream_queries_readtimeout(mock_json, mock_json_txt, response_read_timeout, response_200,global_config):
    logger, config_file, session, client = global_config
...
@mock.patch.object(powerQueriesPull_generic, "json_to_txt")
@mock.patch.object(powerQueriesPull_generic, "stream_to_json")
def test_stream_queries_404_timeout_200(mock_json, mock_json_txt, response_404, response_200, response_read_timeout, global_config):
    logger, config_file, session, client = global_config
    mock_object = MockFactory(status = [response_404, response_read_timeout, response_200],stream=True)
    mock_json.side_effect = mock_object.side_effects
    mock_json_txt.return_value = None
    req = stream_queries_request(session, client, client.stream_list, yearid=99)
    args = mock_json.call_args_list
...
@mock.patch.object(powerQueriesPull_generic, "json_to_txt")
@mock.patch.object(powerQueriesPull_generic, "stream_to_json")
def test_stream_queries_retry_exception(mock_json, mock_json_txt, response_retry_exception, response_200, global_config):
    logger, config_file, session, client = global_config
...
def test_stream_to_json_200(mock_session, response_200, global_config):
    ...
    result = stream_to_json(mock_session , client, "query_url", "params", "student", 10000.00, yearid_payload=None)
    assert result == 200
...
@pytest.mark.skip("Skipped because r.raise_for_status()	raises an exception and interrupts the test") 
@mock.patch.object(powerQueriesPull_generic.requests, "session")
