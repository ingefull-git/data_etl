...
def get_config_file(options, logger):
    ...
    if 'NULL' not in options.test:
        ...
        if not os.path.exists(config_file):
            loggering.error_log(msg="Configuration file does not exist in the defined path: {0}".format(config_file), logger=logger)
            
    else:
        site_folder = get_site_folder(options.folder)
        if site_folder:
            rootdir = '/home/{}'.format(site_folder if '/' not in site_folder else site_folder.split('/')[0])
            config_file = '{}/{}.json'.format(rootdir, '.' + site_folder if '/' not in site_folder else '/.'.join(site_folder.split('/')[::-1]))
        else:
            loggering.error_log(msg="Configuration file does not exist on the site folder: {0}".format(config_file), logger=logger)
...
def get_opts_and_args(args):
    ...
    """
    parser = OptionParser()
    parser.add_option('-a', '--attendance',
        action='store_true', dest='attendance', default=False,
        help='Send only attendance messaging files (not including history)')

    parser.add_option('-s', '--single',
        dest='single', default='NULL', type='string',
