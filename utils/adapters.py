...
class PowerSchoolAdapter(AdapterGeneric):
    ...
    def preadapter_token(self, *args, **kwargs):
        ...
        if 'token' in entity and not self.query.request.headers:
            entity_name = 'token'
            params = {'grant_type': 'client_credentials', 'client_id': self.client.clientId, 'client_secret': self.client.clientSecret}
            headers = {'Content-Type': 'application/x-www-form-urlencoded'}
            payload = {}
            request.append({'payload':payload, 'params':params, 'headers':headers, 'url':entity, 'entity_name':entity_name})
        return request
    ...
    def postadapter_token(self, *args, **kwargs):
        ...
        try:
            ...
            self.context['result'][entity] = {'error': 'No access_token in the response: {}'.format(exc)}
    ...
    def preadapter_yearid(self, *args, **kwargs):
        ...
        if 'yearid' in entity:
            ...
            try:
                ...
                loggering.error_log('Invalid roll-over date entered, please format "mm/dd", received: {0}, {1}'.format(rollover_month_day, err), logger=self.logger)
    ...
    def postadapter_numpages(self, *args, **kwargs):
        ...
        self.logger.debug('{} has {} records.'.format(entity_name, self.context['result'][entity_name]))
        return self.context['result']
    ...
    def preadapter_data(self, *args, **kwargs):
        ...
        pagenum = int(dec.Decimal(num_records/ num_pagelimit).quantize(dec.Decimal('0'), rounding=dec.ROUND_UP)) if entity_name in context['num_pages'] else 1
        ...
        json_file_deleted = fp.check_and_delete_file(full_filename=json_filename, logger=self.logger)
        if stream:
            if not (json_file_deleted and txt_file_deleted):
                self.context['result'][entity_name]['error_file'] = list('' if json_file_deleted else json_filename, '' if txt_file_deleted else txt_filename)
        else:
            if not txt_file_deleted:
                self.context['result'][entity_name]['error_file'] = list('' if txt_file_deleted else txt_filename)
        pages = 1 if stream else pagenum
        url = "{}".format(entity)
        self.context['headers'][entity_name] = gi.get_file_type_headers(client=self.client, entity=entity_name, logger=self.logger)
        payload = self.context['payload'][entity_name]
        self.context['result'][entity_name].update({'stream': 0, 'records': 0} if stream else {'pages': 0, 'records': 0})
        self.context['result'][entity_name].update({'payload': self.context['payload'][entity_name]})
        if (json_file_deleted and txt_file_deleted):
            request = [{'payload':payload, 'params': {'page': page, 'pagesize': 0 if stream else context['records_per_page']}, 'url':url, 'entity_name':entity_name, 'stream':stream} for page in range(1, pages + 1)]
    ...
    def postadapter_data_to_txt(self, *args, **kwargs):
        ...
        try:
            if request['stream']:
                chunks_count = fp.stream_to_json(response=response, entity=entity_name, logger=self.logger)
                if chunks_count:
                    records_count = fp.json_to_txt(headers=self.context['headers'][entity_name], entity=entity_name, logger=self.logger)
                self.context['result'][entity_name]['stream'] += 1
                self.context['result'][entity_name]['records'] += records_count
            else:
                data = response.json()['record']
                records_count = fp.pages_to_txt(headers=self.context['headers'][entity_name], data=data, entity=entity_name, logger=self.logger)
                self.logger.debug('Page {0} out of {1} successfully created for {2} with {3} records.'.format(request['params']['page'], self.context['pages'][entity_name], entity_name, records_count))
